...
import de.eonadev.discord.eobot.utils.configuration.Configuration;
import de.eonadev.discord.eobot.utils.configuration.file.YamlConfiguration;
	...
	private void writeNode(PrintWriter writer, HashMap<String, YamlEntry> map, Yaml yaml, int level) {
		...
		for(Entry<String, YamlEntry> en : map.entrySet()) {
			if(en.getValue().isNode()) {
				...
				if(en.getValue().description != null && en.getValue().description.length() > 0) writer.println(prefix+"#"+en.getValue().description.replace("\n", "\n"+prefix+"#"));
				...
				} else writer.println(prefix+en.getKey()+": "+yaml.dump(en.getValue().o).replace("\n", "")+"\n");
			}
		}
	}
